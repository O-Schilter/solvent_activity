...
class SmilesTokenizer(BertTokenizer):
    """Constructs a SmilesTokenizer inheriting from the BertTokenizer
...
class BasicSmilesTokenizer:
    """Run SMILES tokenization:
        - canonicalize SMILES
        - optionally remove atom maps
        - use tokenization specified by regex_pattern
    """

    def __init__(self, regex_pattern: str = SMI_REGEX_PATTERN_ORIGINAL, remove_maps: bool=False) -> None:
        """Construct a BasicSmilesTokenizer.
        Args:
            regex_pattern: a regex pattern. Defaults to SMI_REGEX_PATTERN_ORIGINAL
        """
        self.regex_pattern = regex_pattern
        self.regex = re.compile(self.regex_pattern)
        self.remove_maps = remove_maps

    def tokenize(self, rxn_smiles: str) -> List[str]:
        """Original tokenization. remove_maps indicates whether to remove maps before tokenization"""

        tokens = [token for token in self.regex.findall(rxn_smiles)]
        return tokens
